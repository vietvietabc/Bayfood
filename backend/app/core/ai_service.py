...
def get_restaurant_context(db: Session) -> str:
    ...
    context_parts.append("Giờ mở cửa: 07:00 – 24:00 mỗi ngày")
    context_parts.append("")
    ...
    if bans:
        for ban in bans:
            context_parts.append(f"  • {ban.tenBan}: {ban.sucChua} chỗ ngồi — Vị trí: {ban.viTri or 'N/A'} — Trạng thái: {ban.trangThai}")
        context_parts.append("")
...
Nhiệm vụ của bạn là hỗ trợ khách hàng, tư vấn món ăn, đặt bàn, và cung cấp thông tin nhà hàng một cách chính xác nhất.
{restaurant_context}
"""
    ...
    try:
        ...
        for msg in chat_history:
            ...
            # Gemini yêu cầu tin nhắn đầu tiên trong lịch sử phải là của user
            if not formatted_history and role != "user":
                continue
                
            # Tránh 2 tin nhắn cùng role liên tiếp bằng cách gộp nội dung
